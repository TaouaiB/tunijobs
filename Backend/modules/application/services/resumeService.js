...
let Job;
try {
  Job = require('../../job/models/jobModel');
} catch {
  Job = null;
}
...
const {
  ...
  mapN8nToParsedCoverLetter,
  analyzeCoverLetterByText,
} = require('../../analysis/services/n8nClient');
...
async function parseApplicationDocument(applicationId, documentId = null) {
  ...
  let doc = null;
  if (documentId) {
    doc = application.documents?.find(
      (d) => d._id?.toString() === String(documentId)
    );
  ...
  const filePath =
    path.isAbsolute(doc.url) ? doc.url : path.join(process.cwd(), doc.url);
